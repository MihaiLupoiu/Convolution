...
void convNormalIndex2(int* x, size_t signalStart, size_t signalEnd, int *h, size_t lh, int y[])
{
    size_t i,j;
...
void input_side_conv(int *x, size_t lx, int *h, size_t lh, int *y)
{
    size_t i,j;
    for(i=0;i<lx; i++)
        for(j=0;j<lh;j++)
...
int main(void) {
    ...
    int i;
    filterLength = 100;
    int* H1 = malloc( filterLength*sizeof(int) );
    ...
    int *C = (int *) calloc(tamResultado + block_size, sizeof(int));
...
*/
    ...
    //tic = clock();
    convNormalIndex2(A,0,tamX,H1,filterLength,B);
    input_side_conv(A, tamX, H1, filterLength, C);
    //toc = clock();
